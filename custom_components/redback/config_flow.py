...
from .const import LOGGER, DOMAIN, TEST_MODE
...
STEP_USER_DATA_SCHEMA = vol.Schema(
    {
        ...
        vol.Required("portal_email"): str,
        vol.Required("portal_password"): str,
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    if TEST_MODE:
        redback = TestRedbackInverter(
            client_id=data["client_id"], client_secret=data["client_secret"], session1=clientSession1, site_index=data["site_index"], session2=clientSession2, portalEmail=data["portal_email"], portalPassword=data["portal_password"]
        )
    else:
...
class RedbackConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            ...
            new["client_secret"] = user_input["client_secret"]
            new["portal_email"] = user_input["portal_email"]
            new["portal_password"] = user_input["portal_password"]
            ...
            clientSession1 = async_get_clientsession(self.hass)
            clientSession2 = async_get_clientsession(self.hass)

            if TEST_MODE:
                redback = TestRedbackInverter(
                    client_id=new["client_id"], client_secret=new["client_secret"], session1=clientSession1, site_index=new["site_index"], session2=clientSession2, portalEmail=new["portal_email"], portalPassword=new["portal_password"]
                )
            else:
                redback = RedbackInverter(
                    client_id=new["client_id"], client_secret=new["client_secret"], session1=clientSession1, site_index=new["site_index"], session2=clientSession2, portalEmail=new["portal_email"], portalPassword=new["portal_password"]
